...
void receive_buff(void)
{
    if((uart_rx_buff[0] == 0xb8) && (uart_rx_buff[1] == 0xae) )
    {
        if(uart_rx_buff[2] == 'O')
        {
            crc_16_val_O.check_sum = CRC16_MODBUS(uart_rx_buff_old, 5);
            crc_16_val_R.check_sum = CRC16_MODBUS(uart_rx_buff, 5);
            if((  crc_16_val_R.b[0] == crc_16_val_O.b[0]) && (  crc_16_val_R.b[1] == crc_16_val_O.b[1]))
            {
                rx_buff();
            }
            else
            {
                uart_rx_buff_old[0] = uart_rx_buff[0];
                uart_rx_buff_old[1] = uart_rx_buff[1];
                uart_rx_buff_old[2] = uart_rx_buff[2];
                uart_rx_buff_old[3] = uart_rx_buff[3];
                uart_rx_buff_old[4] = uart_rx_buff[4];
                uart_rx_buff_old[5] = uart_rx_buff[5];
                uart_rx_buff_old[6] = uart_rx_buff[6];

                uart_tx_buff[0] = uart_rx_buff[0];
                uart_tx_buff[1] = uart_rx_buff[1];
                uart_tx_buff[2] = uart_rx_buff[2];
                uart_tx_buff[3] = uart_rx_buff[3];
                uart_tx_buff[4] = uart_rx_buff[4];
                uart_tx_buff[5] = uart_rx_buff[5];
                uart_tx_buff[6] = uart_rx_buff[6];
            }
        }
    }
}
