...
IFX_INTERRUPT(canIsrRxHandler, 0, ISR_PRIORITY_CAN_RX);                         /* RX Interrupt Service Routine       */
...
void canIsrRxHandler(void)
{
    ...
    blinkLED2();                                 /* Blink LED2 (waiting for a duration of 20 milliseconds) */
}
...
void initMultican(void)
{
    ...
    g_multican.canConfig.nodePointer[TX_INTERRUPT_SRC_ID].priority = ISR_PRIORITY_CAN_TX;   //define the TX interrupt priority
    g_multican.canConfig.nodePointer[RX_INTERRUPT_SRC_ID].priority = ISR_PRIORITY_CAN_RX;   //define the RX interrupt priority
    ...
    IfxMultican_Can_Node_init(&g_multican.canSrcNode, &g_multican.canNodeConfig);             // TX configuration 초기화
    ...
    IfxMultican_Can_Node_init(&g_multican.canDstNode, &g_multican.canNodeConfig);               // RX configuration 초기화
...
void blinkLED1(void)
{
   IfxPort_setPinHigh(g_led.led1.port, g_led.led1.pinIndex);                        /* High the state of the LED      */
   waitTime(IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, WAIT_TIME_10ms));    /* Wait 10 milliseconds           */
   IfxPort_setPinLow(g_led.led1.port, g_led.led1.pinIndex);                         /* Low the state of the LED       */
}
void blinkLED2(void)
{
    IfxPort_setPinHigh(g_led.led2.port, g_led.led2.pinIndex);                        /* High the state of the LED      */
    waitTime(IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, WAIT_TIME_20ms));    /* Wait 20 milliseconds           */
    IfxPort_setPinLow(g_led.led2.port, g_led.led2.pinIndex);                         /* Low the state of the LED       */
}
